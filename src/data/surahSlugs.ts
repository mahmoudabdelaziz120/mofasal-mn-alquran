// Surah slug mapping for mutashabihat routes
...
export function getSlugById(id: number): string {
  return SURAH_SLUGS[id] || `surah-${id}`;
}

export function getIdBySlug(slug: string): number | undefined {
  return Object.entries(SURAH_SLUGS).find(([, s]) => s === slug)?.[0] as unknown as number | undefined;
}
